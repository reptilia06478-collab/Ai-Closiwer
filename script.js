...
/* ═══════════════════════════════════════
   PLUS MENU (ala ChatGPT)
═══════════════════════════════════════ */

var deepThinkMode = false;
...
document.addEventListener('click', function(e) {
    var wrapper = document.querySelector('.plus-menu-wrapper');
...
function openCamera() {
    ...
    /* Create camera input dynamically */
    ...
    input.onchange = function(e) {
        var file = e.target.files[0];
        if (file) handleCameraCapture({ target: { files: [file] } });
        input.remove();
    };
...
function handleCameraCapture(e) {
    ...
    var reader = new FileReader();
    reader.onload = function(ev) {
        showImageAttachment(file, ev.target.result, 'Kamera');
    };
    reader.readAsDataURL(file);
}
...
function handlePhotoPicker(e) {
    ...
    var reader = new FileReader();
    reader.onload = function(ev) {
        showImageAttachment(file, ev.target.result, 'Galeri');
    };
    reader.readAsDataURL(file);
}
...
function openFilePicker() {
    closePlusMenu();
    var input = document.getElementById('fileInputMenu');
    if (input) input.click();
}
...
function handleFilePicker(e) {
    ...
    if (file.type.startsWith('text/') || file.name.match(/\.(txt|md|js|py|html|css|json|xml|csv|log)$/i)) {
        ...
        reader.onload = function(ev) {
            if (typeof state !== 'undefined') {
                state.uploadedFile.content = ev.target.result.slice(0, 10000);
            }
        };
        reader.readAsText(file);
    }
    ...
    showToast('📎 ' + file.name + ' dilampirkan');
}

/* ═══ IMAGE ATTACHMENT PREVIEW ═══ */
function showImageAttachment(file, dataUrl, source) {
    /* Add to upload preview */
    if (typeof state !== 'undefined') {
        state.uploadedFile = file;
        state.uploadedFile.dataUrl = dataUrl;
        state.uploadedFile.isImage = true;
    }
    
    /* Show in upload preview area */
    var preview = document.getElementById('uploadPreview');
    if (preview) {
        preview.innerHTML = 
            '<div style="display: flex; align-items: center; gap: 10px; padding: 8px 14px; width: 100%;">' +
            '<img src="' + dataUrl + '" style="width: 48px; height: 48px; object-fit: cover; border-radius: 8px; border: 2px solid var(--primary);">' +
            '<div style="flex: 1; min-width: 0;">' +
            '<div style="font-size: 12px; font-weight: 700; color: var(--primary);">' + (source === 'Kamera' ? '📷 Kamera' : '🖼️ Galeri') + '</div>' +
            '<div style="font-size: 10px; color: var(--text-muted); overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">' + file.name + '</div>' +
            '</div>' +
            '<button onclick="removeImageAttachment()" style="background: none; border: none; color: var(--error); cursor: pointer; font-size: 20px; padding: 4px 8px;">×</button>' +
            '</div>';
        preview.classList.add('show');
    }
    ...
    if (plusBtn) plusBtn.classList.add('has-attachment');
...
function removeImageAttachment() {
    ...
    if (preview) {
        ...
        preview.innerHTML = '<span>📎</span><span class="file-name" id="fileName"></span><button class="remove-file" onclick="removeFile()">×</button>';
    }
    var plusBtn = document.getElementById('plusBtn');
    if (plusBtn) plusBtn.classList.remove('has-attachment');
    if (typeof updateSendBtn === 'function') updateSendBtn();
}

/* ═══ PLUGIN PANEL ═══ */
function openPluginPanelFromMenu() {
    closePlusMenu();
    setTimeout(function() {
        if (typeof togglePluginPanel === 'function') {
            var panel = document.getElementById('pluginPanel');
            if (panel && !panel.classList.contains('open')) {
                togglePluginPanel();
            }
        } else {
            showToast('⚠️ Plugin panel belum loaded');
        }
    }, 200);
}
...
function toggleDeepThink() {
    ...
    var toggle = document.getElementById('deepThinkToggle');
    var plusBtn = document.getElementById('plusBtn');
    
    if (toggle) {
        if (deepThinkMode) {
            toggle.classList.add('active');
            showToast('🧠 Mode Berpikir Lebih Keras AKTIF');
            if (plusBtn) plusBtn.classList.add('deep-think-active');
        } else {
            toggle.classList.remove('active');
            showToast('🧠 Mode Berpikir Lebih Keras OFF');
            if (plusBtn) plusBtn.classList.remove('deep-think-active');
        }
    }
    
    /* Save to localStorage */
    localStorage.setItem('closiwer_deep_think', deepThinkMode ? '1' : '0');
    
    /* Close menu after delay */
    setTimeout(closePlusMenu, 400);
}

/* Load deep think state on init */
setTimeout(function() {
    var saved = localStorage.getItem('closiwer_deep_think');
    if (saved === '1') {
        deepThinkMode = true;
        var toggle = document.getElementById('deepThinkToggle');
...
/* ═══ INTEGRASI DEEP THINK KE AI ═══ */
/* Wrap sendMessage untuk inject deep think */
var _originalSendMessage = window.sendMessage;
window.sendMessage = function() {
    if (deepThinkMode && typeof config !== 'undefined' && config.apiKey) {
        /* Modify system prompt temporarily */
        if (typeof personalities !== 'undefined' && personalities[state.currentPersonality]) {
            var original = personalities[state.currentPersonality];
            personalities[state.currentPersonality] = original + 
                '\n\n[MODE: BERPIIR LEBIH KERAS]\n' +
                'Berpikirlah lebih dalam dan detail sebelum menjawab. ' +
                'Lakukan step-by-step reasoning. ' +
                'Verifikasi jawaban sebelum mengirim. ' +
                'Berikan analisis yang komprehensif.';
            
            /* Call original */
            var result = _originalSendMessage.apply(this, arguments);
...
console.log('➕ Plus Menu (ala ChatGPT) loaded');
